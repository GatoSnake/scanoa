	...
	public static List<TopicBean> getValuesByIdOA(int id){
	    ...
	    try{
	    	String q = "PREFIX skos: <"+prefix_skos+">\n"
	    			...
	    			+ "PREFIX rdfs: <"+prefix_rdfs+">\n"
	    			+ "PREFIX ContCompTI: <"+prefix_ContCompTI+">\n"
	    			+ "SELECT (STRAFTER(str(?a), '#') as ?subject) (str(?label) as ?object) (str(?b) as ?oaref) \n"
					+ "WHERE { \n"
	...
	public static List<String> getOAsOfTopic(String topico){
	    ...
	    try{
	    	String q = "PREFIX skos: <"+prefix_skos+">\n"
	    			...
	    			+ "PREFIX rdfs: <"+prefix_rdfs+">\n"
	    			+ "PREFIX ContCompTI: <"+prefix_ContCompTI+">\n"
	    			+ "SELECT (str(?oa) as ?oas) \n"
					+ "WHERE { \n"
		    		+ " <"+uri_topico+topico+"> ContCompTI:topicIsReferencedBy ?oa . \n"
		    		+ "}";
//		    System.out.println(q);
	    	QueryExecution qexec = QueryExecutionFactory.create(q,dataset);
	    	ResultSet results = qexec.execSelect();
...
//	    	    	RDFNode object = soln.get("oas");
	...
	public static String getCountTopics(){
		Dataset dataset = ConexionOntology.ConnectModelForRead();
	    String cantTopics = "0";
	    try{
	    	String q = "PREFIX skos: <"+prefix_skos+">\n"
...
//		    System.out.println(q);
	    	...
	    	try{
	    		if (results.hasNext()){
	    			QuerySolution soln = results.nextSolution();
	    			RDFNode subject = soln.get("count");
	    			cantTopics = subject.toString().replace("^^http://www.w3.org/2001/XMLSchema#integer", "");
	    		}
	    	}finally{
	    		qexec.close();
	    	}
